...
{
	CrError_SetString(CrExc_SyntaxError, msg);
	tok->done = E_ERROR;
	return TOK_ERRORTOKEN;
}
...
static char tokenizer_next(TokState* tok)
{
	for (;;)
	{
		...
		if (tok->prompt != NULL)
		{
			char* newtok = CrSys_ReadLine(tok->prompt);
			if (newtok != NULL)
			{
			...
			else if (tok->start != NULL)
			{
				...
				Cr_size_t cur_multi_line_start = tok->multi_line_start - tok->buf;
				char* buf = tok->buf;
...
static void tokenizer_back(TokState* tok, char c)
{
	if (c != EOF)
	{
		if (--tok->cur < tok->buf)
		{
			CrError_SetString(CrExc_SystemError, "tokenizer at beginning of buffer");
			CrError_Print();
		}
		if (*tok->cur != c)
			*tok->cur = c;
		...
		if (tok->type_comments)
		{
			...
			if (!*prefix)
			{
				...
				is_type_ignore = (
					...
					&& !(tok->cur > ignore_end
						&& ((unsigned char)ignore_end[0] >= 128 || Cr_ISALNUM(ignore_end[0]))));
		...
		*p_end = tok->cur;
		...
		if (tok->cur - tok->start == 5 && tok->start[0] == 'a')
		{
			...
			else if (memcmp(tok->start, "async", 5) == 0)
			{
				...
				ahead_tok_kind = CrTokenizer_Get(&ahead_tok, &ahead_tok_start,
					&ahead_tok_end);
...
letter_quote:
	...
	/* Check for two-character token */
	{
		...
		int token = CrToken_TwoChars(c, c2);
		if (token != TOK_OP)
		{
			int c3 = tokenizer_next(tok);
			int token3 = CrToken_ThreeChars(c, c2, c3);
			if (token3 != TOK_OP)
			{
				token = token3;
			}
	...
	*p_start = tok->start;
	*p_end = tok->cur;
	return CrToken_OneChar(c);
}

TokState* CrTokenizer_FromFile(std::ifstream* fp, const char* ps1, const char* ps2)
{
...
int CrTokenizer_Get(TokState* tok, const char** p_start, const char** p_end)
{
...
void CrTokenizer_Free(TokState* tok)
{
	if (tok->encoding != NULL)
		Mem_Free(tok->encoding);
	CrObject_XDECREF(tok->decoding_readline);
	CrObject_XDECREF(tok->decoding_buffer);
	CrObject_XDECREF(tok->filename);
