...
typedef struct _tokstate
{
    ...
    int first_lineno;           // First line of a single line or multi line string expression 
    int level;                  // () [] {} Parentheses nesting level; Used to allow free continuations inside them 
    ...
    const char* multi_line_start; /* pointer to start of first line of
                                     ...
                                     expression */
    CrObject* decoding_readline; // open(...).readline 
    CrObject* decoding_buffer;
    ...
    int async_def_nl;           // =1 if the outermost 'async def' had at least one NEWLINE token after it. 
} TokState;

extern TokState* CrTokenizer_FromFile(std::ifstream* fp, const char* ps1, const char* ps2);

extern int CrTokenizer_Get(TokState* tok, const char** p_start, const char** p_end);
extern void CrTokenizer_Free(TokState* tok);